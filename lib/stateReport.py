...
if len(sys.argv) != 3:
    print "USAGE: stateReport.py inputFile documentFrequencyReport"
    print len(sys.argv)
    sys.exit(2)
...
for a in range(0, len(topics)):
    outLine = "letters-topics " + pad(str(topics[a]), 5)
...
topicDocumentOkay = float(len(okayWords.keys())) / float(len(okayWords.keys()) + len(problemWords.keys()))
                        
print "topic-document okayWords (unique)", len(sorted(okayWords.keys())), topicDocumentOkay                  
...
documentTopicOkay = float(len(okayWords.keys())) / float(len(okayWords.keys()) + len(problemWords.keys()))
                        
print "document-topic okayWords (unique)", len(sorted(okayWords.keys())),   documentTopicOkay                
...
print
print 'overall accuracy', (topicDocumentOkay * documentTopicOkay)

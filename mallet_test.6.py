...
def runToyText(testNbr, testRun, numberOfTopics, topicSize, numberOfTexts, sizeOfTexts, lowNbrTopics, highNbrTopics, malletTopicsNbrs, testResultsFolder, malletFolder, seed, iterations, alpha, beta):
    ...
    for nbrMalletTopics in malletTopicsNbrs:
        ...
        runMalletIterationsAlphaBeta(textsFolder, resultsFolder, fixTrailingSlash(malletFolder), nbrMalletTopics, seed, iterations, alpha, beta)
    ...
    #
    #   RUN TOY TOPIC MODELLER
    #
    
    for nbrMalletTopics in malletTopicsNbrs:

        cmd = "./lib/toyTopicModeller.py '" + textsFolder + "' " + str(nbrMalletTopics) + " " + str(iterations) + " " +  str(alpha) + " " + str(beta) + " " + str(seed) + " '" + resultsFolder + "' > '/dev/null'"
        
        executeCommand(cmd)

        cmd = "./lib/stateReportNoDf.py '" + resultsFolder + "toyTopicModeller.state.txt' > '" + resultsFolder + "toyTopicModeller.stateReport.txt'"

        executeCommand(cmd)
    
    
# ----------------------------------------------------------------------
#   runToyText(testNbr, testRun, numberOfTopics, topicSize, 
#       numberOfTexts, sizeOfTexts, lowNbrTopics, highNbrTopics, 
#       [malletTopicsNbrs], testResultsFolder, malletFolder)
# ----------------------------------------------------------------------

#runToyText(6, 1, 5, 200, 20, 200, 2, 3, [5], "testResults", "/home/spenteco/0/mallet-2.0.7", 1, 1000, 0.1, 0.1)
runToyText(6, 1, 10, 1000, 50, 1000, 2, 5, [10], "testResults", "/home/spenteco/0/mallet-2.0.7", 1, 1000, 0.1, 0.1)
